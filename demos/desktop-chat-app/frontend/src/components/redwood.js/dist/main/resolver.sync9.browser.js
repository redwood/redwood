...
exports.add_version = exports.read = exports.parse_change = exports.resolve_state = exports.create = void 0;
...
function sync9_prune2(x, has_everyone_whos_seen_a_seen_b, has_everyone_whos_seen_a_seen_b2) {
    ...
    function rec(x) {
        ...
        if (x.t == 'val') {
            ...
            if (x.S.nexts.length == 0 && !x.S.next && x.S.elems.length == 1 && is_lit(x.S.elems[0]))
                return x.S.elems[0];
            return x;
        }
        if (x.t == 'arr') {
            ...
            if (x.S.nexts.length == 0 && !x.S.next && x.S.elems.every(is_lit) && !Object.keys(x.S.deleted_by).length)
        ...
        if (x.t == 'str') {
            sync9_space_dag_prune2(x.S, has_everyone_whos_seen_a_seen_b, seen_nodes);
            if (x.S.nexts.length == 0 && !x.S.next && !Object.keys(x.S.deleted_by).length)
                return x.S.elems;
            return x;
        }
    }
    ...
    Object.keys(x.T).forEach(y => {
        if (!seen_nodes[y] && Object.keys(children[y] || {}).some(z => has_everyone_whos_seen_a_seen_b2(y, z)))
            delete_us[y] = true;
    });
    var visited = {};
    var forwards = {};
...
function sync9_add_version(x, vid, parents, changes, is_anc) {
    ...
    changes.forEach(change => {
        ...
        change.keys.forEach((key, i) => {
            if (cur.t == 'val')
                cur = sync9_space_dag_get(prev_S = cur.S, prev_i = 0, is_anc);
            if (cur.t == 'lit') {
                var new_cur = {};
        ...
        if (!change.range) {
            ...
            if (change.val instanceof String && cur.t != 'str')
                throw 'bad';
            if (change.val instanceof Array)
                change.val = change.val.map(x => make_lit(x));
            sync9_space_dag_add_version(cur.S, vid, [[change.range[0], change.range[1] - change.range[0], change.val]], is_anc);
        }
    });
}
exports.add_version = sync9_add_version;
function sync9_read(x, is_anc) {
    if (!is_anc)
        is_anc = () => true;
    if (x && typeof (x) == 'object') {
        if (!x.t)
            return sync9_read(x.val, is_anc);
        if (x.t == 'lit')
            return x.S;
        if (x.t == 'val')
...
exports.parse_change = sync9_parse_change;
    ...
    return new Proxy(x, {
        ...
        set: (x, key, val) => {
            if (typeof (val) == 'string' && typeof (x[key]) == 'string') {
                ...
                if ((x instanceof Array) && key.match(/^\d+$/))
                    key = +key;
    ...
    function rec(x) {
        if (x && typeof (x) == 'object') {
            if (!x.t && x.val) {
                ...
                if (sync9_space_dag_prune(x.S, has_everyone_whos_seen_a_seen_b, seen_nodes))
                    did_something = true;
