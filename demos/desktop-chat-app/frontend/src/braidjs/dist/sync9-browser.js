...
},{"./noConflict":2,"core-js/library/fn/global":17}],2:[function(require,module,exports){
"use strict";
...
},{"base64-js":3,"buffer":4,"ieee754":310}],5:[function(require,module,exports){
...
},{"jsonify":312}],312:[function(require,module,exports){
exports.parse = require('./lib/parse');
exports.stringify = require('./lib/stringify');

},{"./lib/parse":313,"./lib/stringify":314}],313:[function(require,module,exports){
...
function sync9_extract_versions(s9, is_anc) {
  var is_lit = function is_lit(x) {
    return !x || _typeof(x) != 'object' || x.t == 'lit';
  };

  var get_lit = function get_lit(x) {
    return x && _typeof(x) == 'object' && x.t == 'lit' ? x.S : x;
  };

  var versions = [{
    vid: null,
    parents: {},
    changes: [" = ".concat(JSON.stringify(sync9_read(s9, is_anc)))]
  }];
  Object.keys(s9.T).filter(function (x) {
    return !is_anc(x);
  }).forEach(function (vid) {
    var ancs = sync9_get_ancestors(s9, _defineProperty({}, vid, true));
    delete ancs[vid];

    var is_anc = function is_anc(x) {
      return ancs[x];
    };
    ...
    function rec(x) {
      if (is_lit(x)) {} else if (x.t == 'val') {
        sync9_space_dag_extract_version(x.S, s9, vid, is_anc).forEach(function (s) {
          if (s[2].length) changes.push("".concat(path.join(''), " = ").concat(JSON.stringify(s[2][0])));
        });
        sync9_trav_space_dag(x.S, is_anc, function (node) {
          node.elems.forEach(rec);
        });
      } else if (x.t == 'arr') {
        sync9_space_dag_extract_version(x.S, s9, vid, is_anc).forEach(function (s) {
          changes.push("".concat(path.join(''), "[").concat(s[0], ":").concat(s[0] + s[1], "] = ").concat(JSON.stringify(s[2])));
        });
        var i = 0;
        sync9_trav_space_dag(x.S, is_anc, function (node) {
...
function sync9_space_dag_extract_version(S, s9, vid, is_anc) {
  ...
  function add_result(offset, del, ins) {
    if (typeof ins != 'string') ins = ins.map(function (x) {
      return sync9_read(x, function () {
        return false;
      });
    });
...
function sync9_prune2(x, has_everyone_whos_seen_a_seen_b, has_everyone_whos_seen_a_seen_b2) {
  ...
  function rec(x) {
    ...
    if (x.t == 'val') {
      ...
      sync9_trav_space_dag(x.S, function () {
        return true;
      }, function (node) {
    ...
    if (x.t == 'arr') {
      ...
      if (x.S.nexts.length == 0 && !x.S.next && x.S.elems.every(is_lit) && !Object.keys(x.S.deleted_by).length) return {
        t: 'lit',
        S: x.S.elems.map(get_lit)
      };
      return x;
    }
    ...
    if (x.t == 'obj') {
      Object.entries(x.S).forEach(function (e) {
        return x.S[e[0]] = rec(e[1]);
      });

      if (Object.values(x.S).every(is_lit)) {
        var o = {};
        Object.entries(x.S).forEach(function (e) {
          return o[e[0]] = get_lit(e[1]);
        });
        return {
          t: 'lit',
          S: o
  ...
  var delete_us = {};
  var children = {};
  Object.keys(x.T).forEach(function (y) {
    Object.keys(x.T[y]).forEach(function (z) {
      if (!children[z]) children[z] = {};
      children[z][y] = true;
    });
  });
  Object.keys(x.T).forEach(function (y) {
    if (!seen_nodes[y] && Object.keys(children[y] || {}).some(function (z) {
      return has_everyone_whos_seen_a_seen_b2(y, z);
    })) delete_us[y] = true;
  });
  var visited = {};
  var forwards = {};
  ...
  function g(vid) {
    ...
    Object.keys(x.T[vid]).forEach(function (pid) {
      g(pid);

      if (delete_us[vid]) {
        if (delete_us[pid]) Object.assign(forwards[vid], forwards[pid]);else forwards[vid][pid] = true;
  ...
  Object.keys(delete_us).forEach(function (vid) {
    return delete x.T[vid];
  });
  return delete_us;
}
...
function sync9_space_dag_prune2(S, has_everyone_whos_seen_a_seen_b, seen_nodes) {
  ...
  function process_node(node, offset, vid, prev) {
    ...
    var next = node.next;
    var all_nexts_prunable = nexts.every(function (x) {
      return has_everyone_whos_seen_a_seen_b(vid, x.vid);
    });
    ...
    if (Object.keys(node.deleted_by).some(function (k) {
      return has_everyone_whos_seen_a_seen_b(vid, k);
    })) {
      node.deleted_by = {};
    ...
    if (next && !next.nexts[0] && (Object.keys(next.deleted_by).some(function (k) {
      return has_everyone_whos_seen_a_seen_b(vid, k);
    }) || next.elems.length == 0)) {
      node.next = next.next;
      return true;
    ...
    if (nexts.length == 0 && next && !(next.elems.length == 0 && !next.end_cap && next.nexts.length > 0) && Object.keys(node.deleted_by).every(function (x) {
      return next.deleted_by[x];
    }) && Object.keys(next.deleted_by).every(function (x) {
      return node.deleted_by[x];
    })) {
  ...
  while (did_something_this_time) {
    ...
    sync9_trav_space_dag(S, function () {
      return true;
    }, function (node, offset, has_nexts, prev, vid) {
...
function sync9_add_version(x, vid, parents, changes, is_anc) {
  ...
  changes.forEach(function (change) {
    ...
    var cur = x.val;
    if (!cur || _typeof(cur) != 'object' || cur.t == 'lit') cur = x.val = {
      t: 'val',
      S: sync9_create_space_dag_node(null, [cur])
    };
    var prev_S = null;
    var prev_i = 0;
    change.keys.forEach(function (key, i) {
      ...
      if (cur.t == 'lit') {
        ...
        if (cur.S instanceof Array) {
          new_cur.t = 'arr';
          new_cur.S = sync9_create_space_dag_node(null, cur.S.map(function (x) {
            return make_lit(x);
          }));
        } else {
          if (_typeof(cur.S) != 'object') throw 'bad';
          new_cur.t = 'obj';
          new_cur.S = {};
          Object.entries(cur.S).forEach(function (e) {
            return new_cur.S[e[0]] = make_lit(e[1]);
          });
        }
      ...
      if (cur.t == 'obj') {
        var _x = cur.S[key];
        if (!_x || _typeof(_x) != 'object' || _x.t == 'lit') _x = cur.S[key] = {
          t: 'val',
          S: sync9_create_space_dag_node(null, [_x])
        };
        cur = _x;
    ...
    if (!change.range) {
      ...
      if (typeof cur == 'string') {
        ...
        cur = {
          t: 'arr',
          S: sync9_create_space_dag_node(null, cur.S.map(function (x) {
            return make_lit(x);
          }))
        };
        sync9_space_dag_set(prev_S, prev_i, cur, is_anc);
      ...
      if (change.val instanceof Array) change.val = change.val.map(function (x) {
        return make_lit(x);
      });
...
function sync9_read(x, is_anc) {
  if (!is_anc) is_anc = function is_anc() {
    return true;
  };
  ...
  if (x && _typeof(x) == 'object') {
    ...
    if (x.t == 'obj') {
      var o = {};
      Object.entries(x.S).forEach(function (_ref) {
...
function sync9_space_dag_get(S, i, is_anc) {
  var ret = null;
  var offset = 0;
  sync9_trav_space_dag(S, is_anc ? is_anc : function () {
    return true;
  }, function (node) {
...
function sync9_space_dag_add_version(S, vid, splices, is_anc) {
  ...
  var cb = function cb(node, offset, has_nexts, prev, _vid, deleted) {
    var s = splices[si];
    if (!s) return false;
    ...
    if (deleted) {
      if (s[1] == 0 && s[0] == offset) {
        ...
        if (node.elems.length == 0 && !node.end_cap) add_to_nexts(node.nexts, new_node);else sync9_space_dag_break_node(node, 0, undefined, new_node);
        si++;
      }
  ...
  function helper(node, prev, vid) {
    var has_nexts = node.nexts.find(function (next) {
      return f(next.vid);
    });
    var deleted = Object.keys(node.deleted_by).some(function (vid) {
      return f(vid);
    });
    if (cb(node, offset, has_nexts, prev, vid, deleted) == false) throw exit_early;
    ...
    try {
      for (var _iterator = node.nexts[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
...
function sync9_trav_space_dag(S, f, cb, view_deleted, tail_cb) {
  ...
  function helper(node, prev, vid) {
    ...
    if (view_deleted || !Object.keys(node.deleted_by).some(function (vid) {
      return f(vid);
    })) {
    ...
    try {
      for (var _iterator2 = node.nexts[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
    ...
    if (node.next) helper(node.next, node, vid);else if (tail_cb) tail_cb(node);
  }
...
function sync9_parse_change(change) {
  ...
  while (m = re.exec(change)) {
    if (m[1]) ret.keys.push(m[1]);else if (m[2] && m[4]) ret.range = [JSON.parse(m[3]), JSON.parse(m[4].substr(1))];else if (m[2]) ret.keys.push(JSON.parse(m[2]));else if (m[7]) ret.val = JSON.parse(m[7]);
  }
...
function sync9_diff_ODI(a, b) {
  ...
  for (var i = 0; i < d.length; i++) {
    if (d[i][0] == 0) {
      ...
      if (prev) prev[2] += d[i][1];else prev = [offset, 0, d[i][1]];
    } else {
      if (prev) prev[1] += d[i][1].length;else prev = [offset, d[i][1].length, ''];
      offset += d[i][1].length;
    }
  }
...
function sync9_create_proxy(x, cb, path) {
  ...
  var child_path = function child_path(key) {
    return path + '[' + JSON.stringify(key) + ']';
  };

  return new Proxy(x, {
    get: function get(x, key) {
      if (['copyWithin', 'reverse', 'sort', 'fill'].includes(key)) throw 'proxy does not support function: ' + key;
    ...
    set: function set(x, key, val) {
      if (typeof val == 'string' && typeof x[key] == 'string') {
        cb(sync9_diff_ODI(x[key], val).map(function (splice) {
  ...
  function rec(x) {
    if (x && _typeof(x) == 'object') {
      if (!x.t && x.val) {
        ...
        Object.values(x.S).forEach(function (v) {
          return rec(v);
        });
      } else if (x.t == 'arr') {
        if (sync9_space_dag_prune(x.S, has_everyone_whos_seen_a_seen_b, seen_nodes)) did_something = true;
...
function sync9_space_dag_prune(S, has_everyone_whos_seen_a_seen_b, seen_nodes) {
  ...
  function process_node(node, offset, vid, prev) {
    var nexts = node.nexts;
    var next = node.next;
    var first_prunable = nexts.findIndex(function (x) {
      return has_everyone_whos_seen_a_seen_b(vid, x.vid);
    });

    if (first_prunable > 0 && (node.elems.length > 0 || !prev)) {
      first_prunable = nexts.findIndex(function (x, i) {
        return i > first_prunable && has_everyone_whos_seen_a_seen_b(vid, x.vid);
      });
    }
...
function deep_equals(a, b) {
  if (_typeof(a) != 'object' || _typeof(b) != 'object') return a == b;
  ...
  try {
    for (var _iterator3 = ak[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
      var k = _step3.value;
...
},{"./utils":317,"@babel/polyfill":1}],317:[function(require,module,exports){
(function (Buffer){
...
},{"buffer":4,"ethers":309,"json-stable-stringify":311}]},{},[316]);
