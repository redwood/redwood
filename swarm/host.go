...
type Host interface {
	...
	HandleTxReceived(tx tree.Tx, peer Peer)
...
func (h *host) Start() error {
	...
	h.blobStore.OnRefsNeeded(h.handleRefsNeeded)
...
func (h *host) ProvidersOfStateURI(ctx context.Context, stateURI string) <-chan Peer {
	...
	for _, tpt := range h.transports {
		...
		if err != nil {
			h.Warnf("error fetching providers of State-URI %v on transport %v: %v %+v", stateURI, tpt.Name(), err)
			continue
		}
...
func (h *host) HandleTxReceived(tx tree.Tx, peer Peer) {
...
func (h *host) HandleWritableSubscriptionOpened(writeSub WritableSubscription, fetchHistoryOpts *FetchHistoryOpts) {
	...
	if writeSub.Type().Includes(SubscriptionType_States) {
		...
		node, err := h.Controllers().StateAtVersion(writeSub.StateURI(), nil)
...
func (h *host) handleNewState(tx *tree.Tx, node state.Node, leaves []types.ID) {
	...
	go func() {
		...
		if tx.IsPrivate() && tx.ID == tree.GenesisTxID && !h.config.Node.SubscribedStateURIs.Contains(tx.StateURI) {
			ctx, cancel := utils.CombinedContext(h.chStop, 30*time.Second)
			defer cancel()
		...
		wg.Add(3)
		go h.broadcastToWritableSubscribers(ctx, tx, node, leaves, &alreadySentPeers, &wg)
...
func (h *host) broadcastToPrivateRecipients(ctx context.Context, tx *tree.Tx, leaves []types.ID, alreadySentPeers *sync.Map, wg *sync.WaitGroup) {
	defer wg.Done()
...
func (h *host) broadcastToStateURIProviders(ctx context.Context, tx *tree.Tx, leaves []types.ID, alreadySentPeers *sync.Map, wg *sync.WaitGroup) {
	defer wg.Done()
...
func (h *host) SendTx(ctx context.Context, tx tree.Tx) (err error) {
...
func (h *host) AddRef(reader io.ReadCloser) (types.Hash, types.Hash, error) {
	return h.blobStore.StoreObject(reader)
}
...
func (h *host) periodicallyFetchMissingRefs() {
	...
	for {
		...
		case <-tick.C:
			refs, err := h.blobStore.RefsNeeded()
			if err != nil {
