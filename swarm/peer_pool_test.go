...
func TestPeerPool(t *testing.T) {
	...
	peer1.On("DeviceSpecificID").Return("peer1")
	peer2.On("DeviceSpecificID").Return("peer2")
	peer3.On("DeviceSpecificID").Return("peer3")
	peer4.On("DeviceSpecificID").Return("peer4")
		...
		t.Run("maintains invariants under randomized conditions", func(t *testing.T) {
			...
			go func() {
				...
				for j := 0; j < numPeers; j++ {
					...
					peer.On("DeviceSpecificID").Return(fmt.Sprintf("%v", j))
