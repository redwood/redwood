...
func pushCommit(
	...
	logf("PUSH %v %v", txID, parentTxID)

	commit, err := repo.LookupCommit(commitID)
	if err != nil {
		logf("PUSH ERR %v %v: %v", txID, parentTxID, err)
		return
	}
	defer commit.Free()
...
func uploadFilesInCommit(
	...
	for commitFile := range chFiles {
		...
		_, existed := claimedBlobs.LoadOrStore(commitFile.oid.String(), true)
		if existed {
			continue
		}

		var gitBlob *git.Blob
		...
		go func() (err error) {
			...
			case <-ctx.Done():
				return
			case chUploaded <- uploadedFile{commitFile.fullPath, contentType, commitFile.oid.String(), sha3Hash, commitFile.mode}:
			}
			return
		}()
	}
			...
			},
			{
				...
				ValueJSON: mustJSON(M{
					...
					"worktree": map[string]interface{}{
						"Content-Type": "link",
						"value":        "state:" + StateURI + "/commits/" + commitHash + "/files",
					},
				}),
			},
		},
