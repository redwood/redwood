...
function sync9_space_dag_prune2(
  ...
  function process_node(node, offset, vid, prev) {
    ...
    var all_nexts_prunable = nexts.every((x) =>
      has_everyone_whos_seen_a_seen_b(vid, x.vid)
    );
    if (nexts.length > 0 && all_nexts_prunable) {
      var first_prunable = 0;
      var gamma = next;
      if (first_prunable + 1 < nexts.length) {
        gamma = sync9_create_space_dag_node(
          null,
          typeof node.elems == "string" ? "" : []
        );
      ...
      if (first_prunable == 0) {
        if (
          nexts[0].elems.length == 0 &&
          !nexts[0].end_cap &&
          nexts[0].nexts.length > 0
        ) {
          var beta = gamma;
          if (nexts[0].next) {
            beta = nexts[0].next;
    ...
    if (
      Object.keys(node.deleted_by).some((k) =>
        has_everyone_whos_seen_a_seen_b(vid, k)
      )
    ) {
      node.deleted_by = {};
    ...
    if (
      next &&
      !next.nexts[0] &&
      (Object.keys(next.deleted_by).some((k) =>
        has_everyone_whos_seen_a_seen_b(vid, k)
      ) ||
        next.elems.length == 0)
    ) {
...
function sync9_space_dag_add_version(S, vid, splices, is_anc) {
  function add_to_nexts(nexts, to) {
    let i = binarySearch(nexts, function (x) {
