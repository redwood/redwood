...
function resolve_state(state, sender, txID, parents, patches) {
    for (const pStr of patches) {
        const p = parse_change(pStr)
        ...
        if (p.keys.length > 0) {
            ...
            for (let i = 0; i < p.keys.length; i++) {
                ...
                if (m[key] === null || m[key] === undefined) {
                    ...
                    console.log('typeof m[key] !== object ~>', {
                        m,
                        key,
                        state,
                    })
        ...
        if (p.range) {
            ...
            setval = function (val) {
                if (typeof val === 'string') {
                ...
                } else if (getval() === null || getval() === undefined) {
                    old_setval(val)
                } else {
                    ...
                    if (val instanceof Array) {
                        if (getval().length < p.range[1]) {
                            old_setval(
                                [].concat(getval().slice(0, p.range[0]), val),
                            )
                        } else {
                            const x = [].concat(
                                getval().slice(0, p.range[0]),
                                val,
                            )
                            old_setval([].concat(x, getval().slice(p.range[1])))
                        }
                    } else if (getval().length < p.range[1]) {
                        old_setval(
                            [].concat(getval().slice(0, p.range[0]), [val]),
                        )
                    } else {
                        const x = [].concat(getval().slice(0, p.range[0]), [
                            val,
                        ])
                        old_setval([].concat(x, getval().slice(p.range[1])))
                    }
